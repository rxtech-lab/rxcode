...
/// Picks a GitHub repository to register for releases. The release add-repo API
/// needs `installationId` + `repositoryId` + `repositoryFullName`, none of which
/// the release listing carries — so we source the accessible-repo list (which
/// does carry the installation/repository ids) from the secrets
/// `repositories/all` endpoint, the same listing `AddDocsRepoSheet` uses.
struct AddReleaseRepoSheet: View {
    @Environment(AppState.self) private var appState
    @Environment(\.dismiss) private var dismiss

    /// Full names already registered for releases, hidden from the picker.
    ...
    var body: some View {
        NavigationStack {
            content
                .navigationTitle("Add Release Repository")
                .toolbar {
    ...
    private func add(_ repo: SecretsManagedRepo) async {
        ...
        do {
            _ = try await appState.release.addRepository(
                AddReleaseRepoBody(
